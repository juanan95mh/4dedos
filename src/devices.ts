...
let chartHeight = 0
let maxRecordedValue: number = 0
let holdValue = "20mm"
let gripValue = "5 fingers"
let handValue = "right"
...
export function setupDevice(massesElement: HTMLDivElement, outputElement: HTMLDivElement) {
  let isStreaming = true //Pendiente cambiar para que no auto arranque
...
// Create the "Hand" select
const newHandElement = document.createElement("select")
newHandElement.innerHTML = `
  <option value="">Select hand</option>
  <option value="Rigth">Rigth</option>
  <option value="Left">Left</option>
`
newDeviceControlDiv.appendChild(newHandElement)

// Create the "Hold" select
const newHoldElement = document.createElement("select")
newHoldElement.innerHTML = `
...
`
newDeviceControlDiv.appendChild(newHoldElement)

// Create the "Grip" select
const newGripElement = document.createElement("select")
newGripElement.innerHTML = `
  <option value="">Select grip</option>
  <option value="3fingers">3 fingers (extensión)</option>
  <option value="4fingers">4 fingers (semi-arqueo)</option>
  <option value="5fingers">5 fingers (arqueo)</option>
  <option value="fingercurls">Finger curls (solo antebrazo)</option>
`
newDeviceControlDiv.appendChild(newGripElement)
    ...
    // Add event listener for the new hand select
    newHandElement.addEventListener("change", () => {
      handleHandSelection(newHandElement)
    })

    // Add event listener for the new hold select
    newHoldElement.addEventListener("change", () => {
      handleHoldSelection(newHoldElement)
    })
    
    // Add event listener for the new grip select
    newGripElement.addEventListener("change", () => {
      handleGripSelection(newGripElement)
    })
  ...
  `
      ...
      streamButton.id = "stopButton"
      console.log("Start/Stop recording _> Hold: ", holdValue, "Grip: ", gripValue, "Hand: ", handValue)
  ...
  `
      streamButton.addEventListener("click", async () => {
        if (isStreaming) {
          ...
          isStreaming = false
          convertFontAwesome()
          addHistoryData(maxRecordedValue.toString(), holdValue, gripValue,handValue);
          maxRecordedValue = 0;
    ...
    device?.connect(
      async () => {
        ...
        if (
          ...
          isStreaming = true // pendiente cambiar para que no auto arranque
        }
        //addNewDeviceSelect()
      },
      (error: Error) => {
  ...
  /**
   * Handles hold selection and updates the selected hold.
   *
   * @param {HTMLSelectElement} selectElement - The hold select element.
   */
  function handleHoldSelection(selectElement: HTMLSelectElement) {
    holdValue = selectElement.value
    console.log(`Selected hold: ${holdValue}`)
    // Add your logic to handle the selected hold here
  }

  /**
   * Handles grip selection and updates the selected grip.
   *
   * @param {HTMLSelectElement} selectElement - The grip select element.
   */
  function handleGripSelection(selectElement: HTMLSelectElement) {
    gripValue = selectElement.value
    console.log(`Selected grip: ${gripValue}`)
    // Add your logic to handle the selected grip here
  }

  function handleHandSelection(selectElement: HTMLSelectElement) {
    handValue = selectElement.value
    console.log(`Selected hand: ${handValue}`)
    // Add your logic to handle the selected grip here
  }
}
  ...
  if (chart && device !== undefined) {
    ...
    if (!isNaN(numericMass) && !isNaN(numericMax) && !isNaN(numericAverage)) {
      ...
      if (device.id) {
        ...
        maxRecordedValue = Math.max(maxRecordedValue, numericMax);
...
function addHistoryData(max: string, hold: string = "20mm", grip: string = "5 fingers", hand: string = "right") {
  const historyElement = document.querySelector<HTMLDivElement>("#history ul")
  if (historyElement) {
    const listItem = document.createElement("li")
    const currentTime = new Date().toLocaleString()
    listItem.textContent = `${currentTime} -> Hold: ${hold} Grip: ${grip} Max: ${max} kg Hand: ${hand}`
